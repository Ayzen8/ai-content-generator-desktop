...
db.run('DELETE FROM niches', (err) => {
    ...
    niches.forEach((niche, index) => {
        insertNiche.run([niche.name, niche.description, niche.persona, niche.keywords, niche.parent_id], function(err) {
            if (err) {
                console.error(`Error inserting niche ${niche.name}:`, err);
            } else {
                console.log(`✅ Inserted niche: ${niche.name} (ID: ${this.lastID})`);
            }
            
            // Close database after last insert
            if (index === niches.length - 1) {
